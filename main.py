...
parser.add_argument('--feedback', type=float, default=3)
parser.add_argument('--stiefel', type=float, default=0)
...
def main():
    ...
    elif 'FGD' in opt.optim_method:
        param_g = []
        # param_e0 = []
        param_e1 = []

        for key, value in params.items():
            if 'conv' in key and value.dim() == 4 and value.size()[0] <= value.size()[1] and value.size()[2] > 1:  # Stiefel
                param_g.append(value)
                key_g.append(key)
                q = qr_retraction(value.data.view(value.size(0), -1))
                value.data.copy_(q.view(value.size()))
    ...
    def create_optimizer(opt, lr, lrg):
        ...
        elif opt.optim_method == 'FGD':
            dict_g = {'params': param_g, 'lr': lrg, 'momentum': opt.mmtg, 'weight_decay': opt.weightDecay,
                      'stiefel': opt.stiefel, 'feedback': opt.feedback}
            dict_e1 = {'params': param_e1, 'lr': lr, 'momentum': 0.9, 'weight_decay': opt.weightDecay, 'stiefel': 0}
            return SGDStiefel([dict_g, dict_e1])
